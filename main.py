...
def search_node(state: AgentState) -> AgentState:
    if state["iteration_count"] == 0:
        ...
        search_query   = state["query"] + " " + short_critique
    ...
    def generate():
        ...
        for step in agent_app.stream(initial_state):
            ...
            if node_name == "search":
                ...
                yield f"data: {json.dumps({'type':'report','report':state['final_report'],'loops':state['iteration_count'],'confidence':state['confidence_score'],'critiques':state['critiques']})}\n\n"
